...
# XXD Panel 179｜عرض مشاهد مصغرة على شريط عائم
...
- `design-only`: تعرض اللوحة الكاملة ترجمة Panel 179 التصميمية فقط، وتبقى الصورة مرجعاً غير ظاهر.
...
بعد التثبيت أعد تشغيل جلسة Agent ثم استدعِ `$xxd-panel-179`. ويمكن إضافة `--global --agent codex --yes` للتثبيت على مستوى المستخدم.

```text
/xxd-panel-179 photo.jpg --mode top-bottom --size 3:4 --text prompt --locale ar-SA
...
راجع [SKILL.md](SKILL.md) لعقد التشغيل الكامل، ومهايئ التشغيل [بالإنجليزية](references/xxd-panel-179-prompt.en.md) أو [بالصينية](references/xxd-panel-179-prompt.zh-CN.md).
